...
data$Trial <- as.ordered(data$Trial)
# Trial remains numeric (trend modeling)

# Run ART model
m.art <- art(SearchTime ~ Interface * Trial * TaskBlock + Error(ParticipantID), data = data)
...
# Effect sizes
eta_squared_results <- eta_squared(anova_results, generalized = TRUE)
print(format(eta_squared_results, digits=4))

# Contrasts (Interface × TaskBlock, keeping Trial as numeric covariate)
art.con(m.art, ~ Interface, adjust = "bonferroni")
```
...
# Average per Participant × Interface × TaskBlock
# (Trial dropped since we're collapsing across it)
data_avg <- data %>%
  group_by(ParticipantID, Interface, TaskBlock) %>%
  summarise(SearchTime = mean(SearchTime, na.rm = TRUE), .groups = "drop")

# Run ART model (without Trial, since it's averaged out)
m.art.avg <- art(SearchTime ~ Interface * TaskBlock + Error(ParticipantID), data = data_avg)
...
```{r}
# Assuming your cleaned long-format data frame is called df_clean
# and it has columns: SearchTime, RetrieveTime, DepositTime
