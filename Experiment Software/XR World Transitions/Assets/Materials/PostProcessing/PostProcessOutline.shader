Shader "Hidden/Custom/PostProcessOutline"
{
    HLSLINCLUDE
    #include "Packages/com.unity.postprocessing/PostProcessing/Shaders/StdLib.hlsl"

    TEXTURE2D(_MainTex, sampler_MainTex);
    ...
    v2f Vert(AttributesDefault v)
    {
        ...
        o.uv = TransformTriangleVertextToUV(v.vertex.xy);
    ...
    float4 Frag(v2f i) : SV_Target
    {
        ...
        float depth = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sampler_CameraDepthTexture, i.screen_pos.xy);

        // Four sample UV points
        float offset_positive = +ceil(_OutlineThickness * 0.5);
        float offset_negative = -ceil(_OutlineThickness * 0.5);
